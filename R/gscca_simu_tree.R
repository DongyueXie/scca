...
#' @description This function generate tree from data each time instead of using same tree for all simulations
...
gscca.simu=function(niter,u,v,n=50,data.type='normal',
  ...
  for(j in 1:niter){
    ...
    if(is.null(edgex)&is.null(edgey)&is.null(Sx)&is.null(Sy)){
      treex=nj(dist(t(x)))
      Sx=GenStrucMat(x,treex,p_c,dis,p_d,h,wcorr,thresh)
      treey=nj(dist(t(y)))
      Sy=GenStrucMat(y,treey,p_c,dis,p_d,h,wcorr,thresh)
    }
