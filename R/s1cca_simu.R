#' Simulation for s1cca
#' @export

s1cca_simu = function(niter,u,v,n=50,data.type='normal',
                      sigma_z=1,sigma_x=NULL,sigma_y=NULL,
                      theta=0.01,n_x=1000,n_y=1000,lambda_z=10,verbose=F){
  ...
  for(iter in 1:niter){
    ...
    result=CCA(datax$x,datax$y,trace = F)
    corrs[iter]=result$cors
  ...
  return(list(n=n,p=p,q=q,corr=corrs,us=us,vs=vs,
